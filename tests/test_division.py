...
def setup_blender():
    ...
    oriented_cell = cellsA.create_cell(
        "A3", (0, 0, 0), color=(0.5, 0.5, 0), scale=(2, 1.7, 1.7)
    )
...
def test_divide_bisect_cell_volume(setup_blender: tuple[Cell, Cell]):
    ...
    assert mother_large.volume() + daughter_large.volume() == pytest.approx(
        mother_volume_large, rel=2e-1
    )

    assert mother_small.volume() + daughter_small.volume() == pytest.approx(
        mother_volume_small, rel=3e-1
    )
...
def test_divide_oriented_cell(setup_blender: Cell):
    ...
    assert pytest.approx(daughter1.volume(), abs=1e-3) == pytest.approx(
        daughter2.volume(), abs=1e-3
    )
