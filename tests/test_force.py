...
from goo.force import Force, AdhesionForce, MotionForce, create_force, create_adhesion, ForceCollection


@pytest.fixture
def setup_blender():
    bpy.ops.wm.read_factory_settings(use_empty=True)  # Reset to empty scene
    adhesion_force = create_adhesion(strength=100, 
                                     name="AdhesionForce",
                                     loc=(0, 0, 0), 
                                     shape="POINT",)
...
def test_motion_force_set_loc(setup_blender):
    ...
    new_loc = Vector((1, 1, 1))
    target_loc = Vector((2, 2, 2))
    motion_force.set_loc(new_loc, target_loc)
    assert motion_force.loc == new_loc
...
def test_add_remove_force_collection(setup_blender):
    ...
    force_collection.add_force(force)
    assert len(force_collection.forces) == 1
    force_collection.remove_force(force)
    assert len(force_collection.forces) == 0
