...
def divide_boolean(obj): 
    ...
    if 'adhesion force' in obj: 
        print(f"Name of adhesion force: {obj['adhesion force']}")
...
def make_cell(
    ...
    bpy.context.object['previous_pressure'] = 0.001
    bpy.context.object.modifiers['Cloth'].settings.use_pressure_volume = True
    bpy.context.object.modifiers['Cloth'].settings.target_volume = 1
    bpy.context.object.modifiers['Cloth'].settings.pressure_factor = 2
    bpy.context.object.modifiers['Cloth'].settings.fluid_density = 1.05
...
def add_turbulence_motion(type, cells=None, strength=-1000, which='all'): 
    if which == 'all': 
        cells = [
            obj for obj in bpy.context.scene.objects
            if (
                obj.get('object') == 'cell' and
                obj.users_collection[0].get('type') == type
            )
        ]    
    elif which == 'single': 
        cells = cells    

    print(f"Cells to add turbulence: {cells}")

    seeds = np.random.randint(0, 1000, size=len(cells))
    for idx, cell in enumerate(cells): 

        collection = cell.users_collection[0]

        # Check if turbulence already exists for this cell
        turbulence_exists = False
        for obj in collection.objects: 
            if obj.get('object') == 'effector' and obj.name.startswith(cell.name):
                turbulence_exists = True
                break

        # If turbulence already exists, move to the next cell
        if turbulence_exists:
            continue
        ...
        turbulence.field.size = 10
        turbulence.field.noise = 10
        turbulence.field.seed = seeds[idx]
        ...
        turbulence.field.use_global_coords = False
        turbulence['object'] = 'turbulence'
        euler_rotation = np.random.uniform(-2*math.pi, 2*math.pi, size=3)
        turbulence.rotation_euler = euler_rotation

        # Create Material for Turbulence Force Field
        turbulence_material = bpy.data.materials.new(name=f"{name}_Material")
        turbulence.active_material = turbulence_material
        turbulence_material.use_nodes = True
        nodes = turbulence_material.node_tree.nodes
        links = turbulence_material.node_tree.links
        ...
        texture_coord_node.location = (-200, 200)

        # Create Noise Texture node
        noise_texture_node = nodes.new(type='ShaderNodeTexNoise')
        noise_texture_node.location = (0, 200)

        # Connect Texture Coordinate to Noise Texture
        links.new(texture_coord_node.outputs[3], noise_texture_node.inputs[0])

        # Create Math node to control the scale of the noise
        scale_node = nodes.new(type='ShaderNodeMath')
        scale_node.operation = 'MULTIPLY'
        scale_node.inputs[1].default_value = 50
        scale_node.location = (200, 200)

        # Connect Noise Texture to Math node
        links.new(noise_texture_node.outputs[1], scale_node.inputs[0])

        # Create Output node
        output_node = nodes.new(type='ShaderNodeOutputMaterial')
        output_node.location = (400, 200)

        # Connect Math node to Output node
        links.new(scale_node.outputs[0], output_node.inputs[0])

        collection.objects.link(turbulence)
        bpy.context.collection.objects.unlink(turbulence)
        ...
        plane.name = f"{name}_Visualization_Plane"
        plane.scale = (3, 3, 3)

        # Create Material for Plane
        plane_material = bpy.data.materials.new(name=f"{name}_Visualization_Material")
        plane.data.materials.append(plane_material)
        plane_material.use_nodes = True
        plane_nodes = plane_material.node_tree.nodes
        plane_links = plane_material.node_tree.links

        # Clear default nodes
        for node in plane_nodes:
            plane_nodes.remove(node)

        # Create Texture Coordinate node for Plane
        plane_texture_coord_node = plane_nodes.new(type='ShaderNodeTexCoord')
        plane_texture_coord_node.location = (-200, 200)

        # Create Noise Texture node for Plane
        plane_noise_texture_node = plane_nodes.new(type='ShaderNodeTexNoise')
        plane_noise_texture_node.location = (0, 200)

        # Connect Texture Coordinate to Noise Texture for Plane
        plane_links.new(plane_texture_coord_node.outputs[3], plane_noise_texture_node.inputs[0])

        # Create Output node for Plane
        plane_output_node = plane_nodes.new(type='ShaderNodeOutputMaterial')
        plane_output_node.location = (400, 200)

        # Connect Noise Texture to Output node for Plane
        plane_links.new(plane_noise_texture_node.outputs[1], plane_output_node.inputs[0])


def add_turbulence_motion_old(type, cells=None, strength=-1000, which='all'): 
...
class handler_class:
    ...
    def __init__(self):
        ...
        self.daughters = defaultdict(list)
        self.mother_adhesion_strength = defaultdict(lambda: None)
        self.mother_motion_strength = defaultdict(lambda: None)
        self.falloff = defaultdict(lambda: None)
        self.mother_adhesion = defaultdict(lambda: None)
        self.mother_motion = defaultdict(lambda: None)
        self.mother_stiffness = defaultdict(lambda: None)
        self.cells_division_frame = defaultdict(lambda: None)
        self.daughters_division_frame = defaultdict(lambda: None)
        ...
        self.motion_PIDs = {}
        self.KP_motion = 0.05
        self.KI_motion = 0.000001
        self.KD_motion = 0.5
        self.motion_type = 'directed'
    ...
    def launch_simulation(
        ...
        if motility:
            ...
            # bpy.app.handlers.frame_change_post.append(self.turbulence_motion_handler)
        if adhesion:
        ...
        if (division and division_type == 'volume'):
            ...
            # bpy.app.handlers.frame_change_post.append(self.division_handler_volume_turb)            
        if boundary:
        ...
        if self.cell_under_div is not None: 
            ...
            if dividing_cell:
                ...
                if cloth_modifier:
                    ...
                    # Rescue cell cloth
                    apply_physics(obj=self.daughters.get(dividing_cell), 
                                  stiffness=self.mother_stiffness.get(dividing_cell))
            ...
            d1, d2, mother_adhesion_strength, mother_motion_strength, \
                tmp_falloff, mother_adhesion, mother_motion = \
                divide_boolean(dividing_cell)
            
            d1['motion force'] = None
            d2['motion force'] = None
            d2['adhesion force'] = None
            d2['adhesion force'] = None
            
            division_frame = scene.frame_current
            self.cells_division_frame[mother_name] = division_frame
            print(self.cells_division_frame)

            self.daughters[d1.name] = d1     
            self.daughters[d2.name] = d2 

            self.daughters_division_frame[d1.name] = division_frame
            self.daughters_division_frame[d2.name] = division_frame
            print(self.daughters_division_frame)
            # Delete mother cell from list of daughter cells
            if mother_name in self.daughters:
                del self.daughters[mother_name]

            for cell in self.daughters: 
                # Pass on force parameters to daughter cells  
                self.mother_adhesion_strength[cell] = mother_adhesion_strength
                self.mother_motion_strength[cell] = mother_motion_strength
                self.falloff[cell] = tmp_falloff
                self.mother_adhesion[cell] = mother_adhesion
                self.mother_motion[cell] = mother_motion
                self.mother_stiffness[cell] = mother_stiffness
            print(f"Self.daughters: {self.daughters}")

        # Create a list to store keys to delete
        keys_to_delete = []
        print(f'Cells to delete: {keys_to_delete}')

        for cell in self.daughters: 
            cell_object = self.daughters.get(cell)
            print(f"Self.daughter: {cell}")
            if (
                self.daughters_division_frame[cell] is not None
                and scene.frame_current - self.daughters_division_frame[cell] == 1
            ): 

                if cell_object:
                    cloth_modifier = cell_object.modifiers.get("Cloth")
                    if not cloth_modifier:
                        # Toggle back on physics after mesh separation
                        apply_physics(obj=cell_object, 
                                      stiffness=self.mother_stiffness.get(cell))
                
            if (
                self.daughters_division_frame[cell] is not None
                and scene.frame_current - self.daughters_division_frame[cell] == 2
            ): 
                
                # Add adhesion forces to daugther cells
                if (
                    self.mother_adhesion[cell] 
                    and cell_object.get('adhesion force') is None
                ): 
                    print('Mother adhesion detected, passing it on')
                    add_homo_adhesion(cell_name=cell_object.name, 
                                      strength=self.mother_adhesion_strength.get(cell))

                    for cell in self.daughters:
                        cells_same_type = get_cells_same_type(cell_object)
                        for cell_same_type in cells_same_type:
                            missing_forces = get_missing_adhesion_forces(cell_same_type, 
                                                                         cell_object)
                            print(f"Missing adhesion forces: {missing_forces}")
                            for force in missing_forces:
                                cell_object.users_collection[0].objects.link(force)

                if (
                    self.mother_motion[cell] 
                    and cell_object.get('motion force') is None
                ): 
                    print('Mother motion detected, passing it on')
                    add_motion(effector_name=cell_object.name,
                               strength=self.mother_motion_strength.get(cell))
                
                # Add key to delete to the list
                keys_to_delete.append(cell)

                print(f"-- Finishing division of {cell} "
                      f"at frame {scene.frame_current}")

        # Remove the keys from self.daughters
        for key in keys_to_delete:
            del self.daughters[key]             
        
    def division_handler_volume_turb(self, scene, despgraph):
        """
        Handler responsible for cell division based on volume criteria.
        ...
        if self.cell_under_div is not None: 
            ...
            print(f'Mother motion flag: {mother_motion}')
            
            division_frame = scene.frame_current
            print(f"Frame from division: {scene.frame_current - division_frame}")
            ...
            for cell in self.daughters: 
                ...
                self.mother_motion[cell] = mother_motion
                self.mother_stiffness[cell] = mother_stiffness
                self.cells_division_frame[cell] = division_frame

        for cell_name in self.daughters: 
            print(cell_name)
            if (
                self.cells_division_frame[cell_name] is not None
                and scene.frame_current - self.cells_division_frame[cell_name] == 1
            ): 

                cell_object = self.daughters.get(cell_name)
                if cell_object:
                    cloth_modifier = cell_object.modifiers.get("Cloth")
                    if not cloth_modifier:
                        # Toggle back on physics after mesh separation
                        apply_physics(obj=self.daughters.get(cell_name), 
                                      stiffness=self.mother_stiffness.get(cell_name))
                
            if (
                self.cells_division_frame[cell_name] is not None
                and scene.frame_current - self.cells_division_frame[cell_name] == 2
            ): 

                cell_obj = bpy.data.objects[cell_name]
                # Create turbulence velocity field for daugthers
                add_turbulence_motion(type=cell_obj.users_collection[0].get('type'), 
                                      cells=[cell_obj], 
                                      strength=-2000, 
                                      which='single')
    ...
    def initialize_motion_PID(self, cell):
        cell_name = cell.name
        if cell_name not in self.cell_PIDs:
            cloth_modifier = cell.modifiers.get('Cloth')
            if cloth_modifier:
                initial_com = calculate_com(cell)
                pid_scale = 60
                self.KP = 0.05
                self.KI = 0.000001
                self.KD = 0.5
                self.mu = 0
                self.sigma = 1
                self.dz = np.random.normal(loc=self.mu, 
                                           scale=self.sigma, 
                                           size=3)

                self.motion_PIDs[cell_name] = {
                    'integral': 0,
                    'previous_error': 0,
                    'initial_target_loc': initial_com + self.dz,
                    'kp': self.KP,
                    'ki': self.KI,
                    'kd': self.KD,
                    'mu': self.mu,  # in microns per frame
                    'sigma': self.sigma,
                    'next_target_loc': initial_com, 
                    'pid_scale': pid_scale, 
                    'initial_loc': initial_com
                }
    ...
    def growth_PID_handler(self, scene, depsgraph):
        for cell in bpy.context.scene.objects:
            if cell.get('object') == 'cell' and cell.modifiers.get('Cloth'):
                ...
                # save motion path of cells
                com = get_centerofmass(cell)
                self.motion_path[cell.name].append(tuple(com)[:3])
    ...
    def motion_PID_handler(self, scene, depsgraph):
        for cell in bpy.context.scene.objects:
            if cell.get('object') == 'cell' and cell.modifiers.get('Cloth'):
                
                # Check if PID-related properties are initialized for the cell
                self.initialize_motion_PID(cell)
                properties = self.motion_PIDs[cell.name]
                mu = properties['mu']
                ...
                # save motion path of cells
                com = get_centerofmass(cell)
                self.motion_path[cell.name].append((com[0], com[1]))  

                # Calculate current position
                current_position = calculate_com(cell)
                cell['cell_position'] = current_position
                target_position = \
                    current_position + np.random.normal(loc=mu, scale=sigma, size=2)
                cell['target_position'] = properties['previous_target_loc']
                
                print("Selected random motion!")
                next_position = target_position * np.sqrt(self.dt_physics) 
                properties['next_target_loc'] = next_position
                ...
                print(
                    f"Location deviation: {loc_deviation}; "
                )
                ...
                force_name = cell.get('motion force')
                force_obj = bpy.data.objects.get(force_name)
                force_obj.location = new_loc

                # Update previous error and pressure for the next iteration
                properties['previous_error'] = error
                properties['previous_loc'] = new_loc
                cell['previous_loc'] = new_loc

                print(f"New location for {cell_name}: {new_loc}")
    ...
    def turbulence_motion_handler(self, scene, depsgraph):

        # Get all turbulence force fields in the scene
        turbulence_force_fields = [obj for obj 
                                   in bpy.data.objects 
                                   if obj.get('object') == 'turbulence']
        
        print(f'All turbulent forces: {turbulence_force_fields}')

        # Randomly change the location of each turbulence force field
        for turbulence in turbulence_force_fields:
            # Generate random rotation angles in radians for each axis
            rotation_x = np.random.uniform(-2*math.pi, 2*math.pi)
            rotation_y = np.random.uniform(-2*math.pi, 2*math.pi)
            rotation_z = np.random.uniform(-2*math.pi, 2*math.pi)
                
            # Update the rotation of the turbulence force field
            turbulence.rotation_euler = (rotation_x, rotation_y, rotation_z)
    ...
    def random_translate_cells(self, scene, depsgraph): 
        cells = [
        ...
        for cell in cells: 
            ...
            dz = np.random.uniform(-strength, strength)
            
            # Apply translation to the cell object
            '''cell.location.x += dx
            cell.location.y += dy
            cell.location.z += dz'''
            ...
            # Apply translation to the cell object using bpy.ops.transform.translate()
            bpy.ops.transform.translate(value=(dx, dy, dz), orient_type='GLOBAL')
            bpy.ops.object.mode_set(mode='EDIT')
            bpy.ops.object.mode_set(mode='OBJECT')

            # Update the scene to trigger collision detection
            bpy.context.view_layer.update()

            print(f"New cell location: {cell.location}")
